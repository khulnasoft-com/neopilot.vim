...
function! neopilot#Enabled() abort
  ...
  let neopilot_filetypes_disabled_by_default = get(g:, 'neopilot_filetypes_disabled_by_default') || get(b:, 'neopilot_filetypes_disabled_by_default')

  if !get(neopilot_filetypes, &filetype, !neopilot_filetypes_disabled_by_default)
    return v:false
  endif
...
function! neopilot#AcceptNextWord() abort
  let current_completion = s:GetCurrentCompletionItem()
...
function! neopilot#AcceptNextLine() abort
  let current_completion = s:GetCurrentCompletionItem()
  ...
  if exists('b:_neopilot_completions')
    ...
    try
      ...
      if get(response, 'code', v:null) isnot# v:null
        ...
        call neopilot#log#Error(response_text)
        call neopilot#log#Error('stderr: ' . join(a:err, ''))
...
function! s:RenderCurrentCompletion() abort
  ...
  call neopilot#RedrawStatusLine()

  if mode() !~# '^[iR]'
    return ''
  endif
  if !get(g:, 'neopilot_render', v:true)
    return
  endif
  ...
  for part in parts
    ...
    if row != line('.')
      call neopilot#log#Warn('Ignoring completion, line number is not the current line.')
      continue
    endif
    ...
    if has('nvim')
      ...
      let priority = get(b:, 'neopilot_virtual_text_priority',
                  \ get(g:, 'neopilot_virtual_text_priority', 65535))
      ...
      call nvim_buf_set_extmark(0, nvim_create_namespace('neopilot'), row - 1, 0, data)
    else
      if part.type ==# 'COMPLETION_PART_TYPE_INLINE'
...
function! neopilot#Complete(...) abort
  ...
  for buf in loaded_buffers
    if buf.bufnr != current_bufnr && getbufvar(buf.bufnr, '&filetype') !=# ''
      call add(other_documents, neopilot#doc#GetDocument(buf.bufnr, 1, 1))
    endif
  endfor

  let data = {
        \ 'metadata': neopilot#server#RequestMetadata(),
        \ 'document': neopilot#doc#GetDocument(bufnr(), line('.'), col('.')),
...
function! neopilot#DebouncedComplete(...) abort
  call neopilot#Clear()
  if get(g:, 'neopilot_manual', v:false)
    return
  endif
  let current_buf = bufnr('')
  let delay = get(g:, 'neopilot_idle_delay', 75)
  let g:_neopilot_timer = timer_start(delay, function('neopilot#Complete', [current_buf]))
endfunction

function! neopilot#CycleOrComplete() abort
  if s:GetCurrentCompletionItem() is v:null
    call neopilot#Complete()
  else
    call neopilot#CycleCompletions(1)
  endif
endfunction
...
function! s:LaunchChat(out, err, status) abort
  let l:metadata = neopilot#server#RequestMetadata()
  ...
  let l:browser = neopilot#command#BrowserCommand()
...
function! s:GetProjectRoot() abort
  ...
  while l:dir != l:last_dir
    for l:root_hint in g:neopilot_workspace_root_hints
...
function! neopilot#RefreshContext() abort
  " current buffer is 1
  try
    call neopilot#server#Request('RefreshContextForIdeAction', {'active_document': neopilot#doc#GetDocument(1, line('.'), col('.'))})
  catch
    call neopilot#log#Exception()
  endtry
endfunction
...
let s:neopilot_workspace_indexed = v:false
function! neopilot#AddTrackedWorkspace() abort
  if (!neopilot#Enabled() || s:neopilot_workspace_indexed)
    return
  endif
  let s:neopilot_workspace_indexed = v:true
  try
    call neopilot#server#Request('AddTrackedWorkspace', {'workspace': s:GetProjectRoot()})
  catch
    call neopilot#log#Exception()
  endtry
endfunction

function! neopilot#Chat() abort
  if (!neopilot#Enabled())
    return
  endif
  try
    call neopilot#RefreshContext()
    call neopilot#server#Request('GetProcesses', neopilot#server#RequestMetadata(), function('s:LaunchChat', []))
    call neopilot#AddTrackedWorkspace()
    ...
    let chat_ports = get(g:, 'neopilot_port_config', {})
...
function! neopilot#GetStatusString(...) abort
  let s:using_neopilot_status = 1
  if (!neopilot#Enabled())
    return 'OFF'
  endif
  if mode() !~# '^[iR]'
  ...
  if exists('b:_neopilot_status') && b:_neopilot_status > 0
    if b:_neopilot_status == 2
      if exists('b:_neopilot_completions') &&
            \ has_key(b:_neopilot_completions, 'items') &&
            \ has_key(b:_neopilot_completions, 'index')
        if len(b:_neopilot_completions.items) > 0
          return printf('%d/%d', b:_neopilot_completions.index + 1, len(b:_neopilot_completions.items))
        else
          return ' 0 '
        endif
      endif
    endif
    if b:_neopilot_status == 1
      return ' * '
    endif
    return ' 0 '
  endif
...
function! neopilot#RedrawStatusLine() abort
  if s:using_neopilot_status
    redrawstatus
  endif
endfunction

function! neopilot#ServerLeave() abort
  if !exists('g:neopilot_server_job') || g:neopilot_server_job is v:null
    return
  endif

  if has('nvim')
    call jobstop(g:neopilot_server_job)
  else
    call job_stop(g:neopilot_server_job)
  endif
endfunction
