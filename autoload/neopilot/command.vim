...
function! neopilot#command#XdgConfigDir() abort
...
function! neopilot#command#HomeDir() abort
  let data_dir = $XDG_DATA_HOME
  if empty(data_dir)
    let data_dir = $HOME . '/.neopilot'
  else
    let data_dir = data_dir . '/.neopilot'
  endif
  return data_dir
endfunction

function! neopilot#command#LoadConfig(dir) abort
...
let s:api_key = get(neopilot#command#LoadConfig(neopilot#command#HomeDir()), 'apiKey', '')
...
function! s:commands.Auth(...) abort
  if !neopilot#util#HasSupportedVersion()
    ...
    echoerr 'This version of Vim is unsupported. Install ' . min_version . ' or greater to use Neopilot.'
    return
  endif

  let config = get(g:, 'neopilot_server_config', {})
  let portal_url = get(config, 'portal_url', 'https://www.neopilot.khulnasoft.com')
  ...
  if has_key(config, 'api_url') && !empty(config.api_url)
    ...
    let register_user_url = 'https://api.neopilot.khulnasoft.com/register_user/'
  endif
  ...
  if !empty(api_key)
    ...
    let config_dir = neopilot#command#HomeDir()
    let config_path = config_dir . '/config.json'
    let config = neopilot#command#LoadConfig(config_dir)
...
" Run neopilot server only if its not already started
function! neopilot#command#StartLanguageServer() abort
  if !get(g:, 'neopilot_server_started', v:false)
    call timer_start(0, function('neopilot#server#Start'))
    let g:neopilot_server_started = v:true
  endif
endfunction
...
function! neopilot#command#ApiKey() abort
  if s:api_key == ''
    echom 'Neopilot: No API key found; maybe you need to run `:Neopilot Auth`?'
  endif
  return s:api_key
endfunction
