...
if neopilot#util#IsUsingRemoteChat()
  let s:server_port = 42100
endif

let g:neopilot_server_job = v:null
...
function! s:RequestServerStatus() abort
  call neopilot#server#Request('GetStatus', {'metadata': neopilot#server#RequestMetadata()}, function('s:HandleGetStatusResponse'))
endfunction
...
function! s:HandleGetStatusResponse(out, err, status) abort
  ...
  if a:status == 0
    ...
    call neopilot#log#Error(join(a:err, "\n"))
  endif
endfunction
...
function! neopilot#server#Start(...) abort
  let user_defined_neopilot_bin = get(g:, 'neopilot_bin', '')

  if user_defined_neopilot_bin != '' && filereadable(user_defined_neopilot_bin)
    let s:bin = user_defined_neopilot_bin
    call s:ActuallyStart()
    return
  endif
  let user_defined_os = get(g:, 'neopilot_os', '')
  let user_defined_arch = get(g:, 'neopilot_arch', '')
  ...
  if has_key(config, 'portal_url') && !empty(config.portal_url)
    ...
    if v:shell_error == '0'
      ...
      call neopilot#log#Error('Failed to fetch version from ' . config.portal_url)
      call neopilot#log#Error(v:shell_error)
    endif
  endif

  let sha = get(neopilot#command#LoadConfig(neopilot#command#XdgConfigDir()), 'sha', s:language_server_sha)
  let bin_dir = neopilot#command#HomeDir() . '/bin/' . sha
  ...
  if !filereadable(s:bin)
    ...
    if sha ==# s:language_server_sha
      ...
      if has_key(config, 'portal_url') && !empty(config.portal_url)
        ...
        let base_url = 'https://github.com/khulnasoft-com/neopilot/releases/download'
      endif
      ...
      let url = 'https://storage.googleapis.com/khulnasoft-com-dist/neopilot/' . sha . '/language_server_' . bin_suffix . '.gz'
    endif
...
function! s:UnzipAndStart(status) abort
  if has('win32')
    ...
    if !executable('gzip')
      call neopilot#log#Error('Failed to extract language server binary: missing `gzip`.')
      return ''
    endif
  ...
  if !filereadable(s:bin)
    call neopilot#log#Error('Failed to download language server binary.')
    return ''
  endif
  call s:ActuallyStart()
endfunction
...
function! s:ActuallyStart() abort
  ...
  let args = [
        \ s:bin,
        \ '--api_server_url', get(config, 'api_url', 'https://server.neopilot.khulnasoft.com'),
  ...
  if has_key(config, 'api_url') && !empty(config.api_url)
    ...
    let args += ['--portal_url', get(config, 'portal_url', 'https://neopilot.example.com')]
  endif
  if !neopilot#util#IsUsingRemoteChat()
    let args += ['--manager_dir', manager_dir]
  endif
  " If either of these is set, only one vim window (with any number of buffers) will work with Neopilot.
  " Opening other vim windows won't be able to use Neopilot features.
  ...
  if has('nvim')
    let g:neopilot_server_job = jobstart(args, {
                \ 'on_stderr': { channel, data, t -> neopilot#log#Info('[SERVER] ' . join(data, "\n")) },
                \ })
  else
    let g:neopilot_server_job = job_start(args, {
                \ 'out_mode': 'raw',
