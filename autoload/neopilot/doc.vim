...
function! neopilot#doc#GetDocument(bufId, curLine, curCol) abort
  ...
  if empty(filetype) && get(g:, 'neopilot_warn_filetype_missing', v:true)
    call neopilot#log#Warn('No filetype detected. This will affect completion quality.')
    let g:neopilot_warn_filetype_missing = v:false
  endif
  let editor_language = empty(getbufvar(a:bufId, '&filetype')) ? 'unspecified' : getbufvar(a:bufId, '&filetype')
  let doc = {
        \ 'text': join(lines, neopilot#util#LineEndingChars()),
  ...
  let line_ending = neopilot#util#LineEndingChars(v:null)
...
function! neopilot#doc#GetEditorOptions() abort
  return {
      \ 'tab_size': shiftwidth(),
